<?xml version="1.0" encoding="UTF-8" ?>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!-- ExtJS library: all widgets -->
<script type="text/javascript" src="${ctx_webstatic}/framework/extjs-4.1.1/ext-all.js"></script>
<!-- ExtJS library: zh_CN/lang -->
<script type="text/javascript" src="${ctx_webstatic}/framework/extjs-4.1.1/locale/ext-lang-zh_CN.js"></script>
<!-- ExtJS Plugin ux  -->
<script type="text/javascript" src="${ctx_webstatic}/framework/extjs-4.1.1/examples/ux/RowExpander.js"></script>

<script type="text/javascript">
Ext.BLANK_IMAGE_URL = '${cxt_webstyle}/img/blank.gif';

var ctx_webapp = "${ctx_webapp}";
var ctx_webstatic = "${ctx_webstatic}";
var ctx_webstyle = "${cxt_webstyle}";
</script>